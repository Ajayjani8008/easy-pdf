@php
    ...
    $maxSize = $maxSize ?? 51200; // KB, default 50MB
    $uploadUrl = $uploadUrl ?? '/api/upload';
    $fileTypeLabel = $fileTypeLabel ?? 'PDF';
    $uploadButtonText = $uploadButtonText ?? 'Upload ' . $fileTypeLabel;
@endphp
<div 
    x-data="typeof fileUpload === 'function' ? fileUpload() : { isDragging: false, selectedFile: null, uploading: false, uploadedFileId: null, init() {}, handleDrop() {}, handleFileSelect() {}, handleFile() {}, uploadFile() {}, formatFileSize() {} }" 
    x-init="init()" 
    class="bg-white rounded-xl shadow-lg border border-gray-200 p-8"
    data-max-size="{{ $maxSize }}"
    data-upload-url="{{ $uploadUrl }}"
    data-file-type-label="{{ $fileTypeLabel }}"
>
    <div 
    ...
    >
        ...
        <div class="text-gray-500">
            ...
            <p class="text-lg font-medium text-gray-900 mb-2">
                <span x-show="!selectedFile">Drop your {{ $fileTypeLabel }} here, or click to browse</span>
    ...
    <div x-show="selectedFile" x-cloak class="mt-6 flex justify-center">
        <button 
            @click="uploadFile()"
            :disabled="uploading"
        ...
        >
            <span x-show="!uploading" x-cloak>{{ $uploadButtonText }}</span>
            <span x-show="uploading" x-cloak class="flex items-center">
